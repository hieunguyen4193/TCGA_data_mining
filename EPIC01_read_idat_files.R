...
path.to.01.output <- file.path(path.to.main.output, "EPIC_01_output")
...
if (file.exists(file.path(path.to.01.output, "check.csv")) == FALSE){
  ...
  write.csv(data.frame(status = c("finished saving bVals and mVals data to files")),
            file.path(path.to.01.output, "check.csv"))  
} else {
  bVals <- readRDS(file.path(path.to.01.output, "bVals.rds"))
  mVals <- readRDS(file.path(path.to.01.output, "mVals.rds"))
}
...
if (file.exists(file.path(path.to.01.output, "ann.epic.hg19.csv")) == FALSE){
  ...
  write.csv(ann.epic.hg19, file.path(path.to.01.output, "ann.epic.hg19.csv"))
} else {
  ann.epic.hg19 <- read.csv(file.path(path.to.01.output, "ann.epic.hg19.csv"))
}
...
writexl::write_xlsx(resdf, file.path(path.to.01.output, "deconvolution_results_8_Breast_samples.xlsx"))
