...
path.to.02.output <- file.path(path.to.main.output, "02_output", sampling.version)
...
for (group in subset(count.sampledf, count.sampledf$test == "yes")$label){
  ...
  writexl::write_xlsx(DMPs, file.path(path.to.02.output, sprintf("DMP_%s.xlsx", group)))
  writexl::write_xlsx(DMRdf, file.path(path.to.02.output, sprintf("DMR_%s.xlsx", group)))
