...
if (file.exists(file.path(path.to.project.src, "ann450kdf.txt")) == FALSE){
  ann450kdf <- ann450k %>% as.data.frame() %>%
    rownames_to_column("probe") %>%
    rowwise() %>%
    mutate(chrom = str_replace(chr, "chr", "")) %>%
    mutate(start = pos) %>%
    mutate(end = pos + 1)
  ann450kdf <- ann450kdf[, c("chrom", "start", "end")]
  write.table(ann450kdf, file.path(path.to.project.src, "ann450kdf.txt"), quote = FALSE, col.names = FALSE, row.names = FALSE, sep = "\t")  
}
...
count.sampledf <- table(meta.data$label, meta.data$tumor_or_normal) %>% as.data.frame() %>%
  ...
  rowwise() %>%
  mutate(test = ifelse(normal >= 30 & tumor >= 30, "yes", "no"))

#####----------------------------------------------------------------------#####
##### RUN MAIN TEST BY LIMMA, DML
#####----------------------------------------------------------------------#####
for (group in subset(count.sampledf, count.sampledf$test == "yes")$label){
  print(sprintf("working on group %s, test tumor vs control", group))  
  group1 <- tumor.samples[[group]]
  group2 <- normal.samples[[group]]
  ...
  myAnnotation <- cpg.annotate(object = bVals[, input.metadata$sample], datatype = "array", what = "Beta", 
                               ...
                               coef = "tumor - control", arraytype = "450K")
  
  DMRs <- dmrcate(myAnnotation, lambda=1000, C=2, min.cpgs = 5)
  results.ranges <- extractRanges(DMRs)
  DMRdf <- data.frame(results.ranges)
  
  writexl::write_xlsx(DMPs, file.path(path.to.06.output, sprintf("DMP_%s.xlsx", group)))
  writexl::write_xlsx(DMRdf, file.path(path.to.06.output, sprintf("DMR_%s.xlsx", group)))
}
