...
for (group in subset(count.sampledf, count.sampledf$test == "yes")$label){
  if (file.exists(file.path(path.to.08.output, sprintf("DMR_%s.xlsx", group))) == FALSE){
    ...
    writexl::write_xlsx(DMPs, file.path(path.to.08.output, sprintf("DMP_%s.xlsx", group)))
    writexl::write_xlsx(DMRdf, file.path(path.to.08.output, sprintf("DMR_%s.xlsx", group)))
  }
}
